                                ...
                                unsigned char *num_in, size_t data_len, int *err_ret)
{
    ...
    if (data_buf == NULL || enckey_buf == NULL) {
        ESP_LOGE(TAG, "Invalid data or encryption key buffer");
        ret = ATCA_BAD_PARAM;
        goto exit;
    }

    if (target_slot < 0 || target_slot > 15) {
        ESP_LOGE(TAG, "Invalid target slot number %d (must be 0-15)", target_slot);
        ret = ATCA_BAD_PARAM;
        goto exit;
    }

    if (enckey_slot < 0 || enckey_slot > 15) {
        ESP_LOGE(TAG, "Invalid encryption key slot number %d (must be 0-15)", enckey_slot);
        ret = ATCA_BAD_PARAM;
        goto exit;
    }

    if (block < 0 || block > 7) {
        ESP_LOGE(TAG, "Invalid block number %d (must be 0-7)", block);
        ret = ATCA_BAD_PARAM;
        goto exit;
    }
    ...
    ECU_DEBUG_LOG(TAG, "Writing encrypted data to slot %d, block %d using encryption key from slot %d",
                  target_slot, block, enckey_slot);

    // Write encrypted data to slot
    // atcab_write_enc expects:
    // - key_id: target slot number
    // - block: block number within the slot
    // - data: 32-byte data to write
    // - enc_key: 32-byte encryption key
    // - enc_key_id: slot ID where encryption key resides
    // - num_in: optional nonce/input parameter (can be NULL)
    ret = atcab_write_enc(target_slot, block, data_buf, enckey_buf, enckey_slot, num_in);
    if (ret != ATCA_SUCCESS) {
        ESP_LOGE(TAG, "Failed to write encrypted data to slot %d, block %d, returned %02x",
                 target_slot, block, ret);
        ESP_LOGE(TAG, "Hint: Ensure data zone is locked and slot is configured for encrypted writes");
        goto exit;
    }

    ESP_LOGI(TAG, "Successfully wrote %d encrypted bytes to slot %d, block %d", data_len, target_slot, block);
    *err_ret = ret;
    return ESP_OK;
